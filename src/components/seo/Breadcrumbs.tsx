...
export function Breadcrumbs({ items }: { items: BreadcrumbItem[] }) {
  ...
  return (
    <>
      ...
      <nav aria-label="Migas de pan" className="mb-7 text-sm text-slate-400">
            ...
            return (
              <li key={item.href} className="inline-flex items-center gap-1.5">
                ...
                {current ? (
                  ...
                  <Link href={item.href} className="rounded-sm underline-offset-4 hover:text-white hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500">
                    {item.name}
                  </Link>
                )}
              </li>
