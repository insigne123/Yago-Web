    ...
    <Section id="productos" className="z-10">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 -z-10 h-[520px] w-[720px] -translate-x-1/2 -translate-y-1/2 opacity-12 blur-[84px]">
          <div className="absolute inset-0 rounded-full bg-gradient-to-r from-sky-400/25 via-sky-300/15 to-transparent" />
        </div>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          {PRODUCTS.map((p) => {
            ...
            return (
              <div
                key={p.slug}
                className="group hover-lift relative flex flex-col overflow-hidden rounded-[1.8rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))] p-6 shadow-[0_18px_50px_rgba(0,0,0,0.08)] transition hover:border-white/10"
              >
                ...
                <div className="relative mb-6 aspect-[16/9] w-full shrink-0 overflow-hidden rounded-[1.4rem] border border-white/10 bg-white/5">
                  <Image
                ...
                <div className="mt-6 flex flex-wrap items-center gap-2">
                  <Link
                    ...
                    className={`inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-sm text-white transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-200/70 focus-visible:ring-offset-2 focus-visible:ring-offset-background plausible-event-name=Product+More+Info plausible-event-product=${p.slug} plausible-event-location=productos_section`}
                  ...
                  {waHref ? (
                    <a
                      ...
                      className={`inline-flex items-center rounded-full border border-white/10 bg-white px-4 py-2 text-sm font-medium text-[#070b13] transition hover:-translate-y-0.5 plausible-event-name=Product+Contact plausible-event-method=whatsapp plausible-event-product=${p.slug} plausible-event-location=productos_section`}
                    >
                      Contactar
                    </a>
                  ) : (
                    <a
                      href={mailHref}
                      className={`inline-flex items-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white transition hover:bg-white/5 plausible-event-name=Product+Contact plausible-event-method=email plausible-event-product=${p.slug} plausible-event-location=productos_section`}
                    >
                      Contactar
                    </a>
                  )}
