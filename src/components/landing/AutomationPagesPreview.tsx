  ...
  return (
    <Section className="z-10" id="automatizaciones-home">
      <div className="mx-auto max-w-7xl px-4">
        <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))] p-6 shadow-[0_28px_100px_rgba(0,0,0,0.08)] md:p-8">
          <div className="grid gap-8 xl:grid-cols-[0.9fr_1.1fr] xl:items-start">
            <div>
              <Badge variant="outline" className="border-sky-200/20 bg-sky-300/10 px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-sky-300">
              ...
              {AUTOMATION_PAGES.map((page) => {
                ...
                return (
                  <article key={page.slug} className="rounded-[1.6rem] border border-white/10 bg-white/[0.04] p-5">
                    <div className="flex size-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-sky-300">
                    ...
                    <div className="mt-5 rounded-[1.25rem] border border-white/10 bg-white/5 p-4">
                      <div className="text-xs uppercase tracking-[0.16em] text-slate-300">Desde</div>
                      <div className="mt-2 text-2xl font-semibold text-white">{formatUfPrice(featuredPlan.priceUf)}</div>
                      <div className="mt-1 text-sm text-slate-300">{featuredPlan.volume}</div>
                    </div>

                    <Button asChild className="mt-5 w-full rounded-full border border-white/10 bg-white text-[#070b13]">
                      <Link href={`/${page.slug}`}>
                        Ver landing
